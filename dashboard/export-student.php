...
if ($student_id <= 0) {
    header('Location: all-students.php?error=طالب_غير_محدد');
    exit;
}
...
try {
    ...
    if (!$student) {
        header('Location: all-students.php?error=طالب_غير_موجود');
        exit;
    }
    ...
    $sheet->setCellValue('A5', 'تاريخ التسجيل:');
    $sheet->setCellValue('B5', date('Y-m-d', strtotime($student['created_at'])));
    ...
    if (!empty($followups)) {
        ...
        $monthlyFollowups = array_filter($followups, function ($f) use ($monthStart, $monthEnd) {
            return $f['followup_date'] >= $monthStart && $f['followup_date'] <= $monthEnd;
        });
        $monthly_total_100 = 0;
        if (!empty($monthlyFollowups)) {
            $sum = 0;
            $count = 0;
            ...
            $monthly_total_100 = $count ? ($sum / $count) : 0;
        }
        
        $sheet->setCellValue('A' . ($row + 2), 'إجمالي المتابعات:');
        ...
        $sheet->setCellValue('A' . ($row + 6), 'المجموع الشهري (من 100) (' . $selectedYear . '-' . str_pad((string)$selectedMonth, 2, '0', STR_PAD_LEFT) . '):');
    ...
    // إنشاء الملف للتحميل
    $filename = 'تقرير_الطالب_' . str_replace(' ', '_', $student['full_name']) . '_' . date('Y-m-d_H-i-s') . '.xlsx';
    ...
    error_log("Error exporting student: " . $e->getMessage());
    
    // إعادة التوجيه مع رسالة خطأ
    header('Location: student-page.php?id=' . $student_id . '&error=فشل_في_تصدير_التقرير');
    exit;
}
?>