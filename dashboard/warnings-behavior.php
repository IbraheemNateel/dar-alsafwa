...
$page_title = 'انذارات السلوك المتدني';
...
$stmt = $pdo->prepare("
    SELECT s.full_name, df.followup_date, df.day_name, df.behavior_rating
    FROM students s
    JOIN daily_followup df ON s.id = df.student_id
    WHERE df.behavior_rating < 5
      AND df.followup_date BETWEEN ? AND ?
    ORDER BY df.followup_date DESC, df.behavior_rating ASC
");
...
<div class="main-content">
    <header class="content-header">
        <h1>انذارات السلوك المتدني</h1>
        <p class="breadcrumb">لوحة التحكم / انذارات السلوك</p>
    </header>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= $w['behavior_rating'] ?>/10</td>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($warnings)): ?>
                <tr><td colspan="4" style="text-align: center; padding: 2rem;">لا توجد انذارات</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
